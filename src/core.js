...
(function (global) {
  ...
  function findAll(text) {
    ...
    const companyNoQ = /(?<![\wА-Яа-яЁё])(ПАО|ОАО|АО|ООО|ЗАО|НАО|АНО|PAO|OAO|AO|OOO|ZAO|NAO|ANO)[ \t]+([А-ЯЁ][А-ЯЁа-яё\-]{2,40})(?![\wА-Яа-яЁё])/g;
    let cnq;
    ...
    const passportRe = /(?<![\wА-Яа-яЁё])[Пп]аспорт(?:\s+[А-ЯЁ]{2,6})?\s*[:№]?\s*(\d{2}\s?\d{2})\s*№?\s*(\d{6})(?![\d])/g;
    let prm;
