...
(function (global) {
  ...
  function findAll(text) {
    ...
    const companyNoQ = /(?<![A-Za-zА-Яа-яЁё0-9])(ПАО|ОАО|АО|ООО|ЗАО|НАО|АНО|PAO|OAO|AO|OOO|ZAO|NAO|ANO|Банк|Банка|Банке|Банку|Филиал[еауы]?|Представительств[еоау]?)[ \t]+([А-ЯЁ][А-ЯЁа-яё\-]{2,40}(?:[ \t]+[А-ЯЁ][А-ЯЁа-яё\-]{2,40}(?![ \t]*[:№])){0,2})(?![A-Za-zА-Яа-яЁё0-9])/g;
    let cnq;
    ...
    // Компания с ОТКРЫВАЮЩЕЙ кавычкой, но БЕЗ закрывающей — типичная опечатка в
    // реквизитах: «Банк АО «РАЙФФАЙЗЕНБАНК\nБИК ...» (закрывающая `»` потеряна).
    // Применяется только к банковским префиксам и только если в матче есть слово
    // «банк» — иначе слишком высок риск перетянуть случайный текст с одиночной
    // кавычкой.
    const BANK_PREFIXES = '(?:Банк[аеиуы]?|АО|ПАО|ОАО|ООО|ЗАО|НАО|АНО|Филиал[еауы]?|Представительств[еоау]?)';
    const companyOpenQ = new RegExp(
      '(?<![A-Za-zА-Яа-яЁё0-9])(' + BANK_PREFIXES + ')' +
      '(?:[ \\t\\xa0]+[^\\s,()«»\\u201C\\u201D\\u201E"]+){0,3}?[ \\t\\xa0]*' +
      OPEN + '([^\\u00AB\\u00BB\\n\\r]{2,80}?)(?=[\\n\\r]|$)',
      'g'
    );
    let coq;
    while ((coq = companyOpenQ.exec(text)) !== null) {
      const start = coq.index, end = start + coq[0].length;
      if (overlaps(start, end)) continue;
      if (isPublicEntity(coq[0])) continue;
      if (!/[Бб]анк/.test(coq[0])) continue;
      claim(start, end);
      found.push({
        type: 'bank',
        value: coq[0],
    ...
    const labelledPatterns = [
      ...
      { type: 'ks',
        re:  new RegExp(NL + '(К(?:орр?)?(?:\\.?\\s*сч(?:[её]т)?|\\/сч?|\\.?\\s*с\\.?))\\s*[:№]?\\s*(\\d{20})\\b', 'gi') },
      { type: 'rs',
        re:  new RegExp(NL + '(Р(?:асч(?:е|ё)тный)?(?:[\\.\\/\\s]*с(?:ч(?:[её]т)?)?)?)\\s*[:№]?\\s*(\\d{20})\\b', 'gi') },
    ...
    // Для слов-маркеров «почта»/«тел»/«факс» добавляем right-boundary `(?![а-яёА-ЯЁ])`,
    // иначе они срабатывают внутри названий улиц («Почтамтская», «Телеграфная»)
    // и обрывают адрес посередине.
    const addrStop =
      '(?=' +
        '[,\\s]*(?:' +
          '[ИиЁё]?ИНН|КПП|ОГРН|БИК|[ИИ]П\\s|' +
          '[Пп]очта(?![а-яёА-ЯЁ])|[Тт]ел(?![а-яёА-ЯЁ])\\.?|[eE][-‐]?[mM]ail|[Фф]акс(?![а-яёА-ЯЁ])|' +
          '[A-Za-z0-9._+\\-]+@|' +                 // email
          '\\+?[78][\\s\\-\\(]?\\d{3}|' +          // телефон
    ...
    for (const f of found) {
      ...
      // Если сам префикс — это слово «Банк»/«Банка»/... — это уже банк.
      if (f.meta && f.meta.prefix && /^Банк[аеиуы]?$/i.test(f.meta.prefix)) {
        f.type = 'bank';
        continue;
      }
