...
(function (global) {
  ...
  function findAll(text) {
    ...
    // Регистронезависимость для триггеров — через [Хх]-классы на первой букве.
    const fioBefore = new RegExp(
      '(?:' +
        '[Вв]\\s+лице(?:\\s+\\S+){0,6}?\\s+|' +
        '(?:[Гг]енеральн[а-яё]+\\s+)?[Дд]иректор[а-яё]*\\s+|' +
        '[Пп]редставител[а-яё]*\\s+|' +
        '[Пп]одписант[а-яё]*\\s+|' +
        '[Гг]ражданин[а-яё]*\\s+(?:[Рр][Фф]\\s+)?|' +
        '[Ии]ндивидуальн[а-яё]+\\s+предпринимател[а-яё]+\\s+|' +  // Индивидуальный предприниматель X
        'ИП\\s+|' +                                                // ИП X
        '[Оо]т\\s+имени\\s+|' +                                    // от имени X
        '[Нн]а\\s+имя\\s+|' +                                      // на имя X
        '[Фф]\\.?\\s*[Ии]\\.?\\s*[Оо]\\.?\\s*[:\\s]\\s*|' +        // ФИО: X   Ф.И.О.: X
        '[Гг]р[\\.\\-]?\\s*[нН][\\.\\s]*\\s*|' +                   // гр-н X, гр. X
        '[Уу]чредител[а-яё]*\\s+|' +                               // учредитель X
        '[Ии]спытуемы[хйм]\\s+' +                                  // испытуемый X (для мед. договоров)
      ')' +
      NAME3,
      'g'
    );
    const fioAfter = new RegExp(
      '(?<![\\wА-Яа-яЁё])' + NAME3 +
      '(?=\\s*,?\\s*(?:[Дд]ействующ[а-яё]+|[Пп]одписав[а-яё]+|\\(?[Пп]аспорт))',
      'g'
    );
